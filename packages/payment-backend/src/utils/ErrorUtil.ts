  ...
  export async function log(
    ...
    try {
      ...
      await fs.promises.writeFile(
        `${PaymentConfiguration.ROOT}/assets/logs/errors/${prefix}_${fileName}.log`,
        content,
        "utf8",
      );
    } catch {}
...
const directory = new Singleton(async () => {
  try {
    await fs.promises.mkdir(`${PaymentConfiguration.ROOT}/assets/logs`);
  } catch {}

  try {
    await fs.promises.mkdir(`${PaymentConfiguration.ROOT}/assets/logs/errors`);
  } catch {}
});
