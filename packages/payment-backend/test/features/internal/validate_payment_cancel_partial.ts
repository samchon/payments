  ...
  const validate =
    (count: number) => async (record: IPaymentWebhookHistory.IHistory) => {
      const check = (record: IPaymentWebhookHistory.IHistory) => {
        TestValidator.equals("cancelled_at")(!!count)(!!record.cancelled_at);
        TestValidator.equals("cancels.length")(count)(record.cancels.length);
        TestValidator.equals("cancels[].amount")((history.price / 5) * count)(
          record.cancels.map((c) => c.price).reduce((a, b) => a + b, 0),
        );
