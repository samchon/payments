...
import { test_api_toss_vbank_payment } from "./test_api_toss_vbank_payment";

export async function test_api_toss_vbank_payment_cancel_partial(
    connection: PaymentAPI.IConnection,
): Promise<void> {
    // 카드 결제하기
    const history: IPaymentHistory = await test_api_toss_vbank_payment(
        connection,
    );
